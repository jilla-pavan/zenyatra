...
export default function Hero() {
  ...
  return (
    <section id="home" ref={ref} className="relative min-h-[calc(100svh-7.5rem)] overflow-hidden flex items-start justify-center pt-[7rem] sm:pt-[8rem] pb-20">
      ...
      <motion.div className="relative z-10 w-full px-6 xl:px-12 text-center" style={{ opacity }}>
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.2 }}>
          <div className="mb-6 flex items-center justify-center gap-3">
            <div className="h-px w-10 bg-[var(--electric)]" />
            <span className="font-mono text-[0.7rem] uppercase tracking-[0.3em] text-[var(--electric)]">Est. Solapur, Maharashtra</span>
          </div>
        </motion.div>
        ...
        >
          Reliable travel from Solapur to every destination, with comfort and care.
        </motion.p>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.7 }} className="mx-auto flex flex-wrap justify-center gap-4">
          <a
            href="#contact"
        ...
        >
          {highlights.map((item) => (
            <div key={item.label} className="flex w-full items-center justify-between rounded-sm border border-[var(--white)]/15 bg-[var(--black-2)] px-4 py-2 text-sm text-[var(--white)]/90 shadow-[0_10px_30px_rgba(0,0,0,0.12)]">
              <span className="inline-flex h-11 w-11 items-center justify-center rounded-sm bg-[var(--electric)]/10 text-[var(--electric)] text-lg">
                {item.icon}
              </span>
              <span className="ml-3 text-left font-semibold uppercase tracking-[0.08em]">{item.label}</span>
            </div>
          ))}
        </motion.div>
        ...
        >
          <div className="text-[0.6rem] uppercase tracking-[0.2em] text-[var(--electric)]">Scroll</div>
